...
function setup_vendor() {
    ...
    export NinjaOS_ROOT="$3"
    if [ ! -d "$NinjaOS_ROOT" ]; then
        echo "\$NinjaOS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NinjaOS_ROOT/$OUTDIR" ]; then
        mkdir -p "$NinjaOS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NinjaOS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$NinjaOS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NinjaOS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NinjOS_ROOT"/vendor/ninja/build/tools/smali/baksmali.jar
        export SMALIJAR="$NinjaOS_ROOT"/vendor/ninja/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "NinjaOS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$NinjaOS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$NinjaOS_ROOT"/"$OUTDIR"/proprietary
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try NinjaOS target
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NinjaOS_ROOT"/"$OUTDIR"/radio
